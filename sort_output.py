...
with open('D:\\UVA\\NLP\\NLP-Tiny-Project\\answers.json', 'r') as sentences_file:
...
for key, sentences in sentences_data.items():
    match_responses = []
    # Loop through each response in answers_data["answers"]
    for response in answers_data.get("answers", []):
        # Check if any sentence from the current sentences list is present in the response
        if any(sentence in response for sentence in sentences):
            # If a match is found, add the response to the matching_responses list
            match_responses.append(response)
    
    # Assign the matching_responses list to the current key in sorted_answers
